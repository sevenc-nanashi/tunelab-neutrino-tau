...
#[cfg(windows)]
const NEUTRINO_SERVER_FILENAME: &str = "neutrino_server.exe";
#[cfg(windows)]
const NEUTRINO_CLIENT_FILENAME: &str = "neutrino_client.exe";

trait Neutrino: std::fmt::Debug + Send {
    fn root_path(&self) -> &Path;
    fn invoke(&mut self, args: &[&str], cancel_token: &Arc<AtomicBool>) -> anyhow::Result<String>;
    fn shutdown(&mut self);

    fn load_voices(&self) -> anyhow::Result<Vec<crate::speaker::VoiceSource>> {
        info!("Loading voice sources");
        let mut speakers = Vec::new();
        let models_path = self.root_path().join("model");
    ...
    fn synthesize_timing(
        ...
        let label_file = write_score_label_file(score)?;
        ...
        let model_dir = self.root_path().join("model").join(voice_id);
        ...
        crate::synthesizer::parse_timing_label_file(&label_data)
    }
    ...
    fn synthesize_f0(
        ...
        let label_file = write_score_label_file(score)?;
        let label_path = label_file.path().to_string_lossy().to_string();
        let generated_label_file = write_timing_label_file(timings)?;
        let generated_label_path = generated_label_file.path().to_string_lossy().to_string();
        let f0_file = tempfile::NamedTempFile::new()
            .map_err(|e| anyhow::anyhow!("Failed to create temporary f0 file: {}", e))?;
        let f0_path = f0_file.path().to_string_lossy().to_string();
...
impl Engine {
    pub fn new(dll_path: std::path::PathBuf) -> anyhow::Result<Self> {
        ...
        let neutrino_path = PathBuf::from(neutrino_path);
        let neutrino = create_neutrino(neutrino_path)?;

        Ok(Self { dll_path, neutrino })
    }
    ...
    fn select_neutrino_path() -> anyhow::Result<PathBuf> {
        ...
        info!(
            "Selected Neutrino executable candidate: {}",
            result.display()
        );
        if !result.exists() {
            error!(
        ...
        if result.file_name().and_then(|n| n.to_str()) != Some(NEUTRINO_FILENAME) {
            error!(
    ...
    pub fn synthesize(
        ...
        let timings = self
            .neutrino
            .synthesize_timing(&payload.voice_id, &score, cancel_token)?;
        ...
        let inferred_f0_values = self.neutrino.synthesize_f0(
            &payload.voice_id,
            &style_score,
            &merged_phonemes,
            cancel_token,
        ...
        let wav_data = self.neutrino.synthesize_waveform(
            &payload.voice_id,
            &waveform_score,
            &merged_phonemes,
...
#[cfg(windows)]
impl Neutrino for WindowsNeutrino {
    fn root_path(&self) -> &Path {
        &self.root_path
    }
    ...
    fn invoke(&mut self, args: &[&str], cancel_token: &Arc<AtomicBool>) -> anyhow::Result<String> {
        self.spawn_server()?;
        let client_path = self.root_path.join("bin").join(NEUTRINO_CLIENT_FILENAME);
        ...
        let child = command_hidden(&client_path)
            .args(args)
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
        ...
        wait_for_neutrino_process(child, "Neutrino client", cancel_token, || {
            self.restart_server().map(|()| {
                "Synthesis cancelled after timeout; Neutrino server restarted".to_string()
            })
        })
    }
...
impl WindowsNeutrino {
    fn spawn_server(&mut self) -> anyhow::Result<()> {
        ...
        let server_path = self.root_path.join("bin").join(NEUTRINO_SERVER_FILENAME);
        ...
        let child = command_hidden(&server_path)
            .spawn()
            .inspect_err(|e| {
    ...
    fn try_shutdown_server(&mut self) -> anyhow::Result<()> {
        ...
        let mut child = command_hidden(&client_path)
            .arg("shutdown")
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
        ...
        loop {
            if let Some(status) = child.try_wait().map_err(|e| {
                ...
                let stdout =
                    parse_neutrino_output(status.success(), &output.stdout, &output.stderr)?;
...
#[cfg(not(windows))]
impl Neutrino for LocalNeutrino {
    fn root_path(&self) -> &Path {
        &self.root_path
    }

    fn invoke(&mut self, args: &[&str], cancel_token: &Arc<AtomicBool>) -> anyhow::Result<String> {
        let neutrino_path = self.root_path.join("bin").join(NEUTRINO_FILENAME);
        info!("Invoking Neutrino with args: {}", args.join(" "));
        if !neutrino_path.exists() {
            error!(
                "Neutrino executable not found at: {}",
        ...
        let child = command_hidden(&neutrino_path)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| anyhow::anyhow!("Failed to execute Neutrino: {}", e))?;

        wait_for_neutrino_process(child, "Neutrino", cancel_token, || {
            Ok("Synthesis cancelled after timeout; Neutrino process killed".to_string())
        })
    }
...
fn create_neutrino(root_path: PathBuf) -> anyhow::Result<Box<dyn Neutrino>> {
    #[cfg(windows)]
    {
        Ok(Box::new(WindowsNeutrino {
            root_path,
            server: None,
        }))
    }
    #[cfg(not(windows))]
    {
        Ok(Box::new(LocalNeutrino { root_path }))
    }
}

fn write_score_label_file(
    score: &crate::neutrino_score::Score,
...
fn write_timing_label_file(
    timings: &[crate::synthesizer::TimingLabel],
) -> anyhow::Result<tempfile::NamedTempFile> {
    let generated_label_file = tempfile::NamedTempFile::new()
        .map_err(|e| anyhow::anyhow!("Failed to create temporary generated label file: {}", e))?;
    ...
    loop {
        if cancel_token.load(Ordering::SeqCst) {
            cancel_deadline.get_or_insert_with(|| {
                info!("Cancellation detected while waiting for {}", process_name);
        ...
        if let Some(status) = child
            .try_wait()
            .map_err(|e| anyhow::anyhow!("Failed to wait for {}: {}", process_name, e))?
        {
            let output = child
                .wait_with_output()
                .map_err(|e| anyhow::anyhow!("Failed to collect {} output: {}", process_name, e))?;
            debug!(
                "{} exited success={} stdout_bytes={} stderr_bytes={}",
                process_name,
                status.success(),
                output.stdout.len(),
                output.stderr.len()
            );
            return parse_neutrino_output(status.success(), &output.stdout, &output.stderr);
        }

        if let Some(deadline) = cancel_deadline {
            if Instant::now() >= deadline {
                error!(
                    "{} did not finish within {:?} after cancellation",
                    process_name, CANCEL_GRACE_PERIOD
                );
                if let Err(e) = child.kill() {
                    error!("Failed to kill {} process: {}", process_name, e);
                } else {
                    info!("Killed {} process after cancellation timeout", process_name);
                }
                let _ = child.wait();
                return Err(anyhow::anyhow!(on_timeout()?));
            }
        }
...
fn parse_neutrino_output(success: bool, stdout: &[u8], stderr: &[u8]) -> anyhow::Result<String> {
    if success {
        let output = String::from_utf8_lossy(stdout).to_string();
        if output.contains("Error: ") || output.contains("Recv failed: ") {
            Err(anyhow::anyhow!("Neutrino error: {}", output))
        } else {
            Ok(output)
        }
    } else {
