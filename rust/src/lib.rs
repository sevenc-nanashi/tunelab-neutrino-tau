...
pub unsafe extern "C" fn neutrino_tau_load_voice_sources_json(
    ...
    match engine.load_voices() {
        Ok(voices) => match serde_json::to_string(&voices) {
            ...
            Err(e) => {
                error!("Failed to serialize voice sources: {}", e);
                if !err.is_null() {
                    let err_msg =
        ...
        Err(e) => {
            error!("Failed to load voice sources: {}", e);
            if !err.is_null() {
...
pub unsafe extern "C" fn neutrino_tau_synthesize(
    ...
    if cancel_token.load(std::sync::atomic::Ordering::SeqCst) {
        info!("Synthesis request was already cancelled before native execution");
        if !err.is_null() {
    ...
    match engine.synthesize(payload_json, &cancel_token) {
        Ok(json) => create_c_string(&json),
        Err(e) => {
            error!("Native synthesis failed: {}", e);
            if !err.is_null() {
